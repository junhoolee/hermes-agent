...
- **Tool-call inversion** (`bridge.py`): Hermes' OpenAI `tools` are wrapped as
  ...
  `PreToolUse` hook denies every non-bridge tool call so the CLI's own
  built-in tools (kept in context only for its billing classifier) never
  actually run. `client.py` pauses the SDK turn at each tool-call boundary,
  returns an OpenAI-shaped `tool_calls` completion, and resumes the *same*
  SDK turn (no new `query()`) once Hermes core's follow-up `create()` call
  delivers the matching `tool` result messages.
