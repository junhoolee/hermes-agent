...
class TestSplitMessages:
    ...
    def test_excludes_tool_messages_from_prior(self, load_plugin_module):
        convert = load_plugin_module("convert")
        messages = [
        ...
        assert all(m.get("role") != "tool" for m in prior)
...
class TestBuildPrompt:
    ...
    def test_tool_messages_excluded_from_replayed_history(self, load_plugin_module):
        convert = load_plugin_module("convert")
        messages = [
            {"role": "system", "content": "sys"},
            {"role": "user", "content": "do a thing"},
            {"role": "assistant", "content": "calling a tool", "tool_calls": [{"id": "1"}]},
            {"role": "tool", "content": "SECRET_TOOL_OUTPUT", "tool_call_id": "1"},
            {"role": "user", "content": "final question"},
        ]
        prompt = convert.build_prompt(messages, bootstrap_max_chars=60000)
        assert "SECRET_TOOL_OUTPUT" not in prompt
